...
def test_process_invalid_inputs():
    ...
    with pytest.raises(
        ValueError, match='Number of channels in input freqs must match the number of channels in the istft.'
    ):
        testee.process(Freqs('Xs', NUM_CHANNELS + 1, NUM_BINS), Hops('xs', NUM_CHANNELS, NUM_SHIFTS))

    with pytest.raises(ValueError, match='Number of bins in input freqs must match the number of bins in the istft.'):
        testee.process(Freqs('Xs', NUM_CHANNELS, NUM_BINS + 1), Hops('xs', NUM_CHANNELS, NUM_SHIFTS))

    with pytest.raises(
        ValueError, match='Number of channels in output hops must match the number of channels in the istft.'
    ):
        testee.process(Freqs('Xs', NUM_CHANNELS, NUM_BINS), Hops('xs', NUM_CHANNELS + 1, NUM_SHIFTS))

    with pytest.raises(
        ValueError, match='Number of shifts in output hops must match the number of shifts in the istft.'
    ):
        testee.process(Freqs('Xs', NUM_CHANNELS, NUM_BINS), Hops('xs', NUM_CHANNELS, NUM_SHIFTS + 1))
