...
This tutorial assumes that you have already gone through this :doc:`one <../file/steering_delay_sum>`. This tutorial
demonstrates how to perform real-time sound source separation by outputting the enhanced sound that come from a given
direction to the default audio output. Headphones are required to avoid feedback.
...
1. Configures the steering delay-and-sum pipeline and the sound cards.

2. Processes the audio in chunks and performs sound source separation on each segment.

By running this script, you can perform real-time sound source separation for a given direction, making it a
practical demonstration of PyODAS2 capabilities for online SSS tasks.

.. include:: ../../../examples/live/steering_delay_sum_example.py
   :literal:
